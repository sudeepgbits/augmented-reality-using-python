...
def part_6():
    ...
    video_file = "ps3-4-a.mp4"
    ad_video = "my-ad.mp4"  # Place your video in the input_video directory
    frame_ids = [355, 555, 725]
    fps = 40

    # Todo: Complete this part on your own.        
    video = os.path.join(VID_DIR, video_file)
    video_generator = ps3.video_frame_generator(video)

    my_ad= os.path.join(VID_DIR, ad_video)
    my_ad_gen = ps3.video_frame_generator(my_ad)

    video_image = video_generator.next()
    ...
    myad_frame = my_ad_gen.next()
    src_points = ps3.get_corners_list(myad_frame)
    ...
    while video_image is not None and myad_frame is not None:

        print "Processing video"

        markers = ps3.find_markers(video_image)

        homography = ps3.find_four_point_transform(src_points, markers)
        image = ps3.project_imageA_onto_imageB(myad_frame, video_image, homography)
